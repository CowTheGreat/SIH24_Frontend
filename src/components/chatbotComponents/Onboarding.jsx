...
const Onboarding = ({ setInput }) => {
  const steps = [
    ...
    { title: "Review expectations", prompt: "Summarize the team's expectations from this project." },
  ];
    ...
    <div className={Classes.onboardingContainer}>
          ...
          <React.Fragment key={index}>
            ...
            >
              <div className={Classes.step}>
                {completedSteps.includes(step.title) ? <TiTick size={20} /> : index + 1}
              </div>
            ...
            {index < steps.length - 1 && (
              <div
                className={`${Classes.stepLine} ${
                  completedSteps.includes(steps[index].title) ? Classes.completedLine : ""
                }`}
              />
            )}
          </React.Fragment>
      ...
      {showCompletionButton && (
        <div className={Classes.completeContainer}>
          ...
          <button className={Classes.completeButton} onClick={handleCompleteClick}>
            Complete
          </button>
        </div>
      )}
